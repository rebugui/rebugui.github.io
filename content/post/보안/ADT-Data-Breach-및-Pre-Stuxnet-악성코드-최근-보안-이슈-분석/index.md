...
이 악성코드는 주로 다음과 같은 기법을 사용합니다:
1.  **Zero-Day 익스플로잇**: 당시 패치되지 않은 Windows 취약점을 이용한 전파.
2.  **PLC 코드 변조**: 제어 로직을 몰래 변경하여 시스템에 물리적 손상을 입힘.
3.  **루트킷(Rootkit) 기술**: 안티바이러스 제품의 감시를 피해 자신의 존재를 숨김.

**과거 악성코드 vs 현대 ICS 악성코드 비교**

| 비교 항목 | Pre-Stuxnet (2007~2008) | 현대 ICS 악성코드 (2020~) |
| :--- | :--- | :--- |
| **주 타겟** | 특정 산업 시설 (추정) | 일반 인프라 및 에너지 섹터 |
| **은폐 기술** | 드라이버 레벨 루트킷 | Living off the Land (LoLbins) |
| **전파 방식** | USB 드라이브 |  |
