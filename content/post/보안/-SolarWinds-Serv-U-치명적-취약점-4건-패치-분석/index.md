...
| 취약점 유형 | 공격 벡터 (Attack Vector) | 영향 (Impact) | 난이도 |
| :--- | :--- | :--- | :--- |
| **인증 우회 (Auth Bypass)** | 네트워크 (Network) | 권한 상승, 시스템 장악 | 낮음 |
| **버퍼 오버플로우 (Buffer Overflow)** | 네트워크 (Network) | 원격 코드 실행 (RCE), 서비스 중단 | 중간 |
| **디렉터리 순회 (Path Traversal)** | 네트워크 (Network) | 임의 파일 읽기/쓰기, 설정 파일 노출 | 낮음 |
| **DoS (서비스 거부)** | 네트워크 (Network) | 서비스 마비, 자원 고갈 | 낮음 |
