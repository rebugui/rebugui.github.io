...
| 비교 항목 | 일반적인 웹 공격 (SQLi, XSS) | SolarWinds WHD RCE (Auth Bypass) | | :--- | :--- | :--- | | **필요한 권한** | 일반 사용자 권한 또는 게스트 권한 | **없음 (No Authentication)** | | **주요 피해 형태** | 데이터 유출, 웹사이트 변조, 클라이언트 공격 | **서버 장악, 내부 네트워크 침투, 랜섬웨어** | | **탐지 난이도** | 중간 (WAF 로그 분석 가능) | **높음 (정상 트래픽으로 위장 가능)** | | **공격 범위** | 해당 웹 애플리케이션 데이터베이스 | **운영체제 전체 및 연결된 내부망** | | **완화 긴급성** | 일반 (주기적 패치) | **긴급 (즉시 패치 필요)** |
