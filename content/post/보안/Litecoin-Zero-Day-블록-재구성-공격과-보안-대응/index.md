...
| 비교 항목 | 정상적인 Reorg (Natural) | 악의적 Reorg (Zero-Day) |
| :--- | :--- | :--- |
| **발생 원인** | 채굴자들의 네트워크 지연(Latency)으로 인한 동시 채굴 | 합의 알고리즘 로직 결함 악용 또는 검증 우회 |
| **빈도** | 드물지만 네트워크 상태에 따라 발생 가능 | 매우 희소하며, 취약점이 존재할 때만 의도적 발생 |
| **� |  |  |
