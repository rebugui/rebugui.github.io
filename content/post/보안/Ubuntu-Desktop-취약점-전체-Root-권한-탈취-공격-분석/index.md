...
| 비교 항목 | 일반적인 권한 상승 (Sudo Brute Force 등) | Ubuntu Desktop 로직 결함 악용 |
| :--- | :--- | :--- |
| **필요 조건** | 사용자 비밀번호 혹은 취약한 Sudo 설정 | 로컬 사용자 권한만 필요 (비밀번호 불필요) |
| **검출 난이도** | 높음 (로그인 실패 기록 남음) | 낮음 (정상적인 시스템 호출로 위장됨) |
| **공격 속도** | 느림 (딕셔너리 공격 시간 소요) | 즉시 (즉각적인 Root 획득) |
| **주요 타겟** | 인증 메커니즘 자체 | 권한 검증 API/Helper Binary |
