...
| 비교 항목 | 일반적인 랜섬웨어 공격 (피싱/취약한 드라이버) | PAM(BeyondTrust) 취약점 악용 공격 |
| :--- | :--- | :--- |
| **초기 진입 난이도** | 중간 (사용자 클릭 유도 필요) | 낮음 (코드상 익스플로잇 가능) |
| **권한 상승 시간** | 느림 (Local Exploit 필요) | 즉시 (System/Admin 권한 획득) |
| **탐지 회피율** | 낮음 (백신이 첨부 파일 탐지) | 매우 높음 (정상 트래픽 위장) |
| **측면 이동(Lateral)** | 제한적 (네트워크 정책에 따름) | 광범위 (PAM이 가진 모든 자격 증명 사용) |
| **방어 복잡도** | 사용자 교육, EPG로 방어 가능 | 패치 미적용 시 차단 거의 불가능 |
