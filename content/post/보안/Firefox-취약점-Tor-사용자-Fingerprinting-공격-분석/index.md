...
| 비교 항목 | 일반적 Fingerprinting (Canvas, Fonts) | 취약점 기반 Fingerprinting (Timing/State) |
| :--- | :--- | :--- |
| **주요 데이터 소스** | 렌더링 결과물 (해시값) | 실행 시간, 메모리 상태, 네트워크 타이밍 |
| **Tor 방어 효과** | 높음 (RFP로 표준화됨) | 낮음 (브라우저 엔진 레벨의 동작 차이) |
| **재현성** | 환경 설정 변경 시 변동 가능 | 브라우저 버전/패치에 의존적이며 안정적 |
| **탐지 난이도** | 비교적 쉬움 (비정상적인 스크립트 패턴) | 어려움 (정상적인 웹 트래픽으로 위장 가능) |
