...
| 비교 항목 | 기존 Chaos 랜섬웨어 | 새로운 Chaos 변종 |
| :--- | :--- | :--- |
| 초기 벡터 | 피싱 이메일, 악성 파일 | Cloud API 노출, Misconfiguration |
| 암호화 방식 | AES-256 기본 | AES-256 + 파일별 독립 키 |
| 네트워크 기능 | 없음 | SOCKS5 Proxy (포트 46542) |
| 측면 이동 | 수동 | 자동화된 내부 스캔 및 이동 |
| 지속성 | 없음 | 백도어 프록시 유지 |
| 타겟 | 개인 사용자 | Cloud 인프라, K8s 클러스터 |
| 감지 난이도 | 중간 | 높음 (정상 트래픽 위장) |
...
| 취약점 유형 | 발생 빈도 | 위험도 | 침투 경로 |
| :--- | :--- | :--- | :--- |
| 과도한 IAM 권한 | 78% | 높음 | API 키 탈취 |
| 노출된 관리 포트 | 65% | 높음 | 직접 접근 |
| 기본 자격 증명 사용 | 52% | 높음 | 무차별 대입 |
| 비활성화된 로깅 | 43% | 중간 | 흔적 삭제 |
| 패치 미적용 | 38% | 중간 | 알려진 취약점 |
