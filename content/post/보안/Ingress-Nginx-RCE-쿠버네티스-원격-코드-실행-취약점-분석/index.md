...
| 구분 | 일반적인 웹 애플리케이션 취약점 | Ingress-Nginx RCE 취약점 | | :--- | :--- | :--- | | **공격 대상** | 특정 웹 컨테이너 (Pod) | 클러스터의 진입점 (Control Plane 구성요소) | | **필요 권한** | 별도의 권한 불필요 (웹 노출) | Ingress 리소스 생성/수정 권한 (RBAC) | | **주요 피해** | 데이터 유출, 웹사이트 변조 | 클러스터 장악, 횡적 이동, 모든 트래픽 도청 | | **탐지 난이도** | 비교적 쉬움 (WAF 등) | 어려움 (정상적인 설정 변경으로 위장됨) |
