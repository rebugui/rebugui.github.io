...
| 권한 스코프 | 탈취 시 가능한 공격 | 피해 등급 |
| :--- | :--- | :--- |
| `repo` (풀 액세스) | 프라이빗 레포지토리 코드 유출, 커밋 조작, 백도어 삽입 | 치명적 |
| `repo:read` | 소스코드 열람, 내부 API 구조 파악 | 높음 |
| `admin:org` | 조직 설정 변경, 멤버 권한 상승 | 치명적 |
| `workflow` | GitHub Actions 워크플로우 수정, CI/CD 파이프라인 탈취 | 치명적 |
| `packages` | 내부 패키지 레지스트리 접근, 종속성 교체 | 높음 |
| `gist` | 비공개 Gist 데이터 유출 | 중간 |
...
| 항목 | 권장 사항 | 우선순위 |
| :--- | :--- | :--- |
| 토큰 유형 | Fine-grained Personal Access Token 사용 | 높음 |
| 권한 범위 | 필요한 최소 레포지토리에만 권한 부여 | 높음 |
| 만료 정책 | 30일 이하로 설정, 자동 갱신 비활성화 | � |
