...
| 비교 항목 | 기존 SharePoint 검색 | M365 Copilot 검색 |
| :--- | :--- | :--- |
| **검색 방식** | 키워드 매칭 (Keyword Matching) | 의도적 의미 이해 (Semantic Understanding) |
| **접근성** | 특정 사이트나 라이브러리 직접 방문 필요 | 전체 테넌트 데이터 동시 크롤링 |
| **컨텍스트** | 문서 단위 독립적 검색 | 문서 간 연결 및 관계성 분석 |
| **위험 요인** | 사용자가 문서 위치를 알아야 함 | 사용자가 권한만 있다면 모든 것을 즉시 파악 |
| **데이터 노출 범위** | 낮음 (Low) | 극도로 높음 (Critical) |
