...
| 비교 항목 | Composer 1 (전작) | 경쟁 모델 (GPT-4 Class) | Composer 2 (신규) | 향상 폭 |
| :--- | :---: | :---: | :---: | :---: |
| **SWE-bench Verified** | 45.2% | 58.0% | **62.5%** | +17.3%p |
| **CursorBench (Pass@1)** | 38.7% | 52.4% | **59.1%** | +20.4%p |
| **추론 속도 (Latency)** | 1.0x (Baseline) | 0.8x | **1.5x** | +50% |
| **비용 효율성** | Low | Very High | **Medium** | High |
...
1.  **명확한 의도 전달**: 파일 경로를 포함하여 구체적인 수정 범위를 지정하세요.
2.  **관련 파일 참조**: 작업에 필요한 유틸리티나 타입 정의가 있다면 `@` 기호 등을 통해 명시적으로 참조하세요.
3.  **반복적인 리팩토링**: 한 번에 큰 변경을 시도하기보다, 컴포넌트 단위로 나누어 점진적으로 리팩토링을 요청하세요.
