...
| 비교 항목 | 전통적 최적화 관점 | 딥러닝 동역학 관점 |
| :--- | :--- | :--- |
| **손실 함수 형태** | 볼록(Convex) 혹은 단순 비볼록 | 복잡한 비볼록, 고차원 다양체 |
| **국소 최솟값** | 나쁜 최적해로 갇힘 위험 | 대부분의 최솟값은 성능이 비슷함 (Flat Minima) |
| **일반화(Generalization)** | 훈련 오차 최소화와 직결 | 암묵적 편향(Implicit Bias)과 지형의 평탄성 중요 |
| **접근 방식** | 해석적 해(Analytic Solution) 추구 | 확률적 경로 및 통계적 물리학 모델링 |
