...
| 한계점 | 범용 도구 | 사내 에이전트 |
| :--- | :--- | :--- |
| 사내 문맥 이해 | ❌ (학습 데이터에 없음) | ✅ (RAG로 실시간 검색) |
| 내부 API/라이브러리 | ❌ (할루시네이션 위험) | ✅ (코드베이스 인덱싱) |
| CI/CD 파이프라인 연동 | ❌ (수동 복사/붙여넣기) | ✅ (자동화된 워크플로우) |
| 보안/컴플라이언스 | ⚠️ (코드 유출 위험) | ✅ (온프레미스 배포 가능) |
| 커스텀 리뷰 규칙 | ❌ | ✅ (팀별 설정 가능) |
...
| 전략 | 설명 | 토큰 절감율 |
| :--- | :--- | :--- |
| 계층적 요약 | 파일 → 디렉토리 → 모듈 단계별 요약 | 60-70% |
| 동적 로딩 | 필요할 때만 코드 로드 | 40-50% |
| 캐싱 | 자주 참조되는 코드 캐싱 | 30-40% |
| 슬라이딩 윈도우 | 최근 수정 이력 기반 컨텍스트 | 20-30% |
