...
2.  **VZ(Virtualization.framework) 설정**: Swift에서 `VZVirtualMachineConfiguration`을 정의하고 `VZDiskImageStorageDeviceAttachment`를 연결한 `VZVirtioBlockDeviceConfiguration`으로 디스크를 구성합니다. CPU와 메모리도 명시적으로 설정합니다.
3.  **Ephemeral 스토리지 구성**: 쓰기 가능한 임시 디스크 이미지를 VM에 연결하고, VM 종료 후 호스트의 정리 절차에서 해당 이미지를 삭제합니다.
4.  **에이전트 인터페이스 개발**: 게스트 내부의 서비스 또는 `VZFileHandleSerialPortAttachment`를 이용한 직렬 포트 연결로 호스트와 메시지를 교환합니다. 통신 채널의 인증·입출력 처리 방식은 구현별로 검증해야 합니다.
5.  **네트워크 격리**: 기본적으로 네트워크 장치를 제공하지 않습니다. 통신이 필요하다면 네트워크 모드와 방화벽·송신 정책을 별도로 설계하고 허용된 대상으로만 연결하세요. `VZBridgedNetworkDeviceAttachment` 자체는 인터넷 접근을 차단하지 않습니다.

이러한 구성은 특히 **LLM Code Interpreter** 기능을 구현할 때 유용합니다. 복잡한 라이브러리 의존성이 필요한 데이터 분석 코드를 사용자의 호스트 컴퓨터를 오염시키지 않고 실행할 수 있기 때문입니다.
