...
다음은 데이터 품질의 중요성을 설명하기 위한 **가상 상황**입니다. SOC 분석가가 많은 정상 알림 속에서 중요한 공격 신호를 놓친다면, 탐지 모델이 학습한 데이터의 편향과 잘못된 라벨도 함께 점검해야 합니다. 특정 조직의 실제 침해 사례나 측정된 알림 비율을 뜻하지 않습니다.
...
딥러닝 기반 보안 모델은 입력 데이터의 분포를 학습합니다. 공격 탐지 모델에서는 정상·악성 데이터의 비율과 라벨 오류가 중요합니다. 극단적으로 불균형한 데이터를 그대로 학습하면 정상으로만 예측해도 전체 정확도가 높게 나올 수 있으며, 실제 공격을 놓치는 거짓 음성(False Negative)을 별도로 평가해야 합니다.
...
아래 표는 **실측 벤치마크가 아닌 정성적 비교 예시**입니다. 데이터셋, 라벨링 절차, 모델, 검증 분할과 탐지 지표가 제시되지 않은 수치를 특정 성능으로 받아들여서는 안 됩니다.

| 비교 항목 | 저품질 원시 로그 (Raw Logs) | 고품질 정제 데이터 (Curated Data) |
| :--- | :--- | :--- |
| **데이터 구조** | 비정형 텍스트, 시간 순서 혼란, 누락값 다수 | 구조화된 스키마, 결측치 처리, 타임스탬프 정렬 |
| **라벨링 정확도** | 검증되지 않은 자동 라벨에는 오류가 남을 수 있음 | 전문가 검토·표본 검사로 오류를 찾아 수정 |
| **클래스 불균형** | 공격 사례가 드물어 정상 예측에 치우칠 수 있음 | 실제 분포와 평가 목적에 맞는 샘플링·가중치 검토 |
| **특징(Feature) 품질** | 단순 로그 메시지에만 의존 | 통계적 특징, 행동 패턴, 운영 맥락 함께 검토 |
| **평가 지표** | 전체 정확도만 보면 공격 누락을 감출 수 있음 | 별도 평가 집합에서 정밀도·재현율과 오탐을 함께 측정 |
