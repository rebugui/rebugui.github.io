...
1.  **가지 A:** 열쇠가 옛 연인의 집 열쇠다 (과거 회상).
2.  **가지 B:** 열쇠가 보물 창고의 열쇠다 (액션 전개).
3.  **가지 C:** 열쇠가 아무 쓸모가 없는 오래된 물건이다 (반전/이터니티).
...
| 비교 항목 | 기존 AI 라이팅 툴 (Generic LLM) | CineGraphs (Qwen2.5-7B FT) |
| :--- | :--- | :--- |
| **학습 데이터** | 일반 인터넷 텍스트, 요약문 | 100편의 고급 영화 구조적 데이터 |
| **출력 형식** | 선형 텍스트 (Paragraph) | **확률적 분기 그래프 (Graph)** |
| **창의성 지표** | 낮음 (Cliché 위험) | 높음 (비선형적, 실험적 구조) |
| **사용자 경험** | 텍스트 프롬프트 입력 후 결과 대기 | **노드 탐색 및 시각적 조각(Sculpting)** |
| **서빙 효율성** | Cloud API 의존 (고비용) | **Single 4090 + vLLM (저비용)** |
