...
- 비밀번호는 제조사 지원 범위와 기관 정책에 맞는 충분한 길이를 확보하고, 알려진 유출·추측 가능 비밀번호를 허용하지 않는지 확인
...
# <계정명>은 승인된 실제 계정명으로 교체하고 운영 영향 검토 후 실행
useradd <계정명>
```
...
2. **침해 시 비밀번호 변경**: 유출 또는 탈취 정황이 확인되면 즉시 변경하고 관련 접속 이력을 조사합니다. 근거 없는 일률적 주기 변경은 보안 권고와 구분해야 합니다.
...
일반 보안 권고로는 길이·유출 비밀번호 차단·시도 제한·MFA를 우선 고려합니다. [NIST SP 800-63B-4](https://pages.nist.gov/800-63-4/sp800-63b.html#passwordver)는 근거 없는 주기 변경과 문자 종류 강제를 요구하지 않습니다. 적용 대상 기관에 별도 심사 기준이 있다면 해당 기준의 판본과 범위를 확인하세요.
