...
**보안 위협 시나리오(가상):**
1. **퍼블릭 S3 버킷**: 실수로 S3 버킷을 퍼블릭으로 설정하면 저장한 민감 정보가 노출될 수 있음
...
**점검 시나리오(가상)**
- 공급업체가 공유 목적으로 공개했던 버킷에 비공개 문서가 함께 저장된 경우, 누구나 문서에 접근할 수 있습니다.
- 테스트용 버킷의 ACL 또는 정책에 전체 사용자 읽기 권한이 남아 있다면 접근 범위와 실제 다운로드 기록을 조사해야 합니다.
- 공개 접근 가능 여부만으로 실제 열람·반출이 확인된 것은 아닙니다. 접근 로그 등으로 사실관계를 구분해 확인하세요.
