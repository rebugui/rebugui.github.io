...
2. **미등록 프로그램 조사 후 변경 승인**
   - 목록에 없는 프로그램도 공급사 드라이버·제어 소프트웨어 의존성·복구 도구일 수 있으므로 즉시 삭제하지 않음
   - 공급사와 운영 담당자가 용도를 확인하고 변경 영향·복구 계획을 승인한 뒤, 불필요한 항목만 시험 환경에서 검증해 제거
...
아래 제거 명령은 승인된 변경 작업에서만 사용합니다. `<패키지명>`은 실제로 제거해도 안전하다고 확인한 패키지명으로 교체하고, 의존성 제거 목록을 먼저 검토하세요.
...
- 제거에 따른 운전 영향과 복구 가능성을 공급사·운영 담당자와 확인
...
제어시스템 내 구성요소의 소프트웨어를 조사해 운영에 불필요한 항목만 공급사·운영 담당자의 영향 검토와 변경 승인을 거쳐 제거해야 합니다. 단지 기존 목록에 없다는 이유만으로 자동 삭제해서는 안 됩니다.
