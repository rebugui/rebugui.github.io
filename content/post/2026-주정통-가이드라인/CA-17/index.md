...
2. **규정 준수**: 적용 대상 법령·계약·기관별 정책의 정확한 보관 요건 확인
3. **포렌식**: 디지털 포렌식을 위한 증거 보존
4. **비용 최적화**: 불필요하게 긴 보관 기간은 스토리지 비용 증가

**점검 시나리오(가상)**
- 사고 조사에 필요한 기간보다 로그를 짧게 보관했다면 발생 경위를 추적할 자료가 부족할 수 있습니다.
- 기관에 실제로 적용되는 별도 보관 의무를 확인하지 않고 기본 보관 기간만 사용하면 감사 요건을 충족하지 못할 수 있습니다.
...
로그별 최소·권장 기간은 법령의 적용 대상, 계약, 기관 정책 및 사고 조사에 필요한 범위에 따라 달라집니다. 모든 CloudTrail 로그를 일률적으로 7년, 보안 로그를 1년 보관해야 한다는 식의 근거 없는 공통 기간은 적용하지 마세요. 조사 목적과 비용을 고려해 로그 유형별로 승인된 정책을 만들고, 서비스 설정값을 그 정책과 대조합니다. 앞의 CLI 숫자는 환경별 **설정 예시**이며 법적 보관 의무가 아닙니다.
...
**로그보관기간설정**은 클라우드 보안 사고 조사와 규정 준수를 위한 필수 항목입니다. 적용 대상 법령·계약과 기관 정책에 요구된 로그 종류·보관 기간을 먼저 확인하고, S3 라이프사이클 및 CloudWatch Logs 보관 정책을 이에 맞춰 설정하세요. 아래의 예시 기간은 보편적인 법적 의무가 아닙니다.

**핵심 액션 아이템**
1. CloudWatch Logs는 로그별 조사·준수 요건에 따라 보관 기간 설정
2. S3 라이프사이클 정책으로 장기 보관 및 비용 최적화
3. CloudTrail/감사 로그의 보관 기간은 기관별 적용 요건을 확인해 결정
